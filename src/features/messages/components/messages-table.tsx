...
type MessageRecord = Awaited<ReturnType<typeof getMessages>>[number];
...
const defaultFormValues: MessageFormValues = {
  content: "",
  clientId: "",
};
...
export function MessagesTable({
  ...
  const [formValues, setFormValues] = useState<MessageFormValues>(defaultFormValues);
  const [isPending, startTransition] = useTransition();

  const tableRows = useMemo(() => messages.slice(0, 6), [messages]);
  ...
  const handleEditClick = (message: MessageRecord) => {
    if (!canEdit) return;
    setEditing(message);
    setFormOpen(true);
  };
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      const action = editing
        ? await updateMessage(editing.id, formValues)
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
            Review and respond to recent client communications.
          </p>
        </div>
        {canEdit ? (
          <Button
